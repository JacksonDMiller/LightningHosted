...
sub.on("invoice_updated", async (invoice) => {
  if (invoice.is_confirmed === true) {
    const doc = await Users.findOne({
      "images.paymentRequest": invoice.request,
    });
    const index = await doc.images.findIndex(
      (image) => image.paymentRequest === invoice.request
    );
...
module.exports = function (app) {
  ...
  app.post("/api/payinvoice/", async (req, res) => {
    try {
      const details = await decodePaymentRequest({
        lnd,
        request: req.body.invoice,
      });
      if (req.user.sats >= details.tokens) {
        ...
        res.status(400).send({ error: `You don't have enough sats` });
      }
    } catch (err) {
      console.log(err);
  ...
  app.get("/api/checkpayment/:invoice", async (req, res) => {
    if (req.params.invoice) {
      try {
        ...
        const doc = await Users.findOne({
          "images.paymentRequest": req.params.invoice,
        });
        if (doc) {
          // find the image assigned to the payment request in the user record
          const index = await doc.images.findIndex(
            (image) => image.paymentRequest === req.params.invoice
          );
          if (doc.images[index].payStatus === true) {
            res
              .status(200)
              .send({ payStatus: true, imageData: doc.images[index] });
          } else {
